...
import { listEventFields, replaceEventFields } from "../services/eventFields.service.js";

export const listEventFieldsHandler: RequestHandler<{ eventId: string }> = async (req, res, next) => {
	try {
...
export const replaceEventFieldsHandler: RequestHandler<{ eventId: string }> = async (req, res, next) => {
	try {
		const eventId = parseId(req.params.eventId);
