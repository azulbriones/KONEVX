...
type PdfRow = {
	...
	createdAt: Date;
	participant: { emailNormalized: string | null; phoneNormalized: string | null };
...
export function sendRegistrationsPdf(
	res: Response,
	eventName: string,
	...
	const drawDocumentHeader = (isFirstPage: boolean = false) => {
		...
		if (isFirstPage) {
			doc.fontSize(10).font("Helvetica-Bold").fillColor(C_PRIMARY).text(`TOTAL DE REGISTROS: ${rows.length}`, MARGIN, startY + 8, { align: "right", width: PAGE_WIDTH });
		}
		doc.y = startY + 45;
	};
	...
	const drawSectionHeader = (groupName: string) => {
		...
		doc.fillColor("#ffffff").font("Helvetica-Bold").fontSize(10).text(`  SECCIÓN: ${String(title).toUpperCase()}`, MARGIN + 5, doc.y + 7);
		doc.moveDown(1.5);
	};
	...
	if (options.groupBy) {
		rows.sort((a, b) => {
			const getVal = (r: PdfRow) => {
				...
				return String(fv?.value ?? "SIN ESPECIFICAR");
			};
	...
	for (const r of rows) {
		...
		if (options.groupBy === "assignedGroup") rowGroupValue = r.assignedGroup || "SIN ASIGNAR";
			...
			const fv = r.fieldValues.find((f) => f.eventField.key === options.groupBy);
			rowGroupValue = String(fv?.value ?? "SIN ESPECIFICAR");
		}
		...
		if (hasNotes) {
			...
			doc.fontSize(7).font("Helvetica-Bold").fillColor(C_DANGER).text("INCIDENCIA / NOTA:", MARGIN + 5, notesY);
			doc.fontSize(8).font("Helvetica-Oblique").fillColor(C_DARK).text(String(r.checkInNotes ?? ""), MARGIN + 85, notesY, { width: PAGE_WIDTH - 95, ellipsis: true });
		}
